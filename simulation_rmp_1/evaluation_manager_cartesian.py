...
class EvaluationManagerCartesian(Node):
    def __init__(self):
        super().__init__('evaluation_manager_cartesian')

                # >>> ADD: Startup banner <<<
        self.get_logger().info("=" * 80)
        self.get_logger().info("EVALUATION MANAGER CARTESIAN - STARTING UP")
        self.get_logger().info("=" * 80)
        ...
        self.pose_pub = self.create_publisher(Pose, '/cartesian_impedance_controller/target_pose', 10)
        ...
        if 'RUN_DIR' in os.environ and os.path.exists(os.environ['RUN_DIR']):
            ...
            self.simulation_cartesian_dir = "/home/matteo/Simulation_cartesian"
            self.run_dir = os.path.join(self.simulation_cartesian_dir, f"Run_{self.run_timestamp}")
    ...
    def wait_for_dependencies(self):
        ...
        while not self.count_subscribers('/planning_scene') > 0:
            time.sleep(1.0)
        while not self.count_subscribers('/cartesian_impedance_controller/target_pose') > 0:
            time.sleep(1.0)
    ...
    def set_target_pose(self, pose):
        """Set target pose for cartesian impedance controller"""
        # REMOVE PoseStamped wrapping - publish Pose directly
        self.pose_pub.publish(pose)
        self.get_logger().info(f"Published target pose: [{pose.position.x:.3f}, {pose.position.y:.3f}, {pose.position.z:.3f}]")
    ...
    def run_single_simulation(self):
        ...
        # ADD THIS: Give time for subscription to receive the message
        self.get_logger().info("Waiting for controller to receive target pose...")
        time.sleep(1.0)  # Increased from 0.5s to 1.0s
        
        # Republish to ensure it's received
        self.set_target_pose(target)
        time.sleep(0.5)
...
def main(args=None):
    ...
    node = EvaluationManagerCartesian()
    node.destroy_node()
    rclpy.shutdown()
